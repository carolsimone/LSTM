...
class LSTM(object):
    ...
    def __init__(self, size: int = None, n_h=100, seq_len=40,
                 epochs=10, lr=0.001, beta1=0.9, beta2=0.999):

        self.size = size  # number of characters in the data set.
        ...
        self.params = {}
        xavier_std = (1.0 / np.sqrt(self.size + self.n_h))  # Xavier initialisation
        # Forget gate layer
        self.params["Wf"] = np.random.randn(self.n_h, self.n_h + self.size) * xavier_std
        self.params["bf"] = np.ones((self.n_h, 1))
        # Input gate layer
        self.params["Wi"] = np.random.randn(self.n_h, self.n_h + self.size) * xavier_std
        self.params["bi"] = np.zeros((self.n_h, 1))
        # Cell gate layer
        self.params["Wc"] = np.random.randn(self.n_h, self.n_h + self.size) * xavier_std
        self.params["bc"] = np.zeros((self.n_h, 1))
        # Output Gate layer
        self.params["Wo"] = np.random.randn(self.n_h, self.n_h + self.size) * xavier_std
        self.params["bo"] = np.zeros((self.n_h, 1))
        # Output
        self.params["Wv"] = np.random.randn(self.size, self.n_h) * (1.0 / np.sqrt(self.size))
        self.params["bv"] = np.zeros((self.size, 1))
        # Adam and gradients
        self.grads = {}
        self.adam_params = {}
        ...
        self.smooth_loss = -np.log(1.0 / self.size) * self.seq_len
        return
    ...
    def _sample(self, h_prev, c_prev, sample_size):
        ...
        for t in range(sample_size):
            ...
            idx = np.random.choice(range(self.size), p=y_hat.ravel())
            x = np.zeros((self.size, 1))
            x[idx] = 1
    ...
    def _forward_backward(self, x_batch, y_batch, h_prev, c_prev):
        ...
        for t in range(self.seq_len):
            x[t] = np.zeros((self.size, 1))
            x[t][x_batch[t]] = 1

            y_hat[t], v[t], h[t], o[t], c[t], c_bar[t], i[t], f[t], z[t] = \
                self._forward_step(x[t], h[t - 1], c[t - 1])
    ...
    def train(self, X, y, verbose=True):
        ...
        for epoch in range(self.epochs):
            ...
            for j in range(0, len(X) - self.seq_len, self.seq_len):
                # prepare batches
                x_batch = [X[j: j + self.seq_len]]
                y_batch = [y[j: j + self.seq_len + 1]]
